...
class MAVLinkGPSHandler:
    def __init__(self, connection_string=MAVLINK_CONNECTION, baud=MAVLINK_BAUD):
        ...
        # NEW: Add altitude tracking for waypoints
        self.waypoint_altitude_mode = "CURRENT"  # Options: "CURRENT", "RELATIVE", "ABSOLUTE"
        self.relative_altitude_offset = 10.0  # meters above current altitude
        self.absolute_altitude = 100.0  # fixed altitude MSL
        
        self.connect()
    
    def set_waypoint_altitude_mode(self, mode, value=None):
        """
        Set how waypoint altitudes are calculated:
        - "CURRENT": Use current aircraft altitude
        - "RELATIVE": Use current altitude + offset (value = offset in meters)
        - "ABSOLUTE": Use fixed altitude MSL (value = altitude in meters)
        """
        self.waypoint_altitude_mode = mode
        if mode == "RELATIVE" and value is not None:
            self.relative_altitude_offset = value
        elif mode == "ABSOLUTE" and value is not None:
            self.absolute_altitude = value
        
        print(f"🏔️ Waypoint altitude mode set to: {mode}")
        if mode == "RELATIVE":
            print(f"   Offset: +{self.relative_altitude_offset}m above current altitude")
        elif mode == "ABSOLUTE":
            print(f"   Fixed altitude: {self.absolute_altitude}m MSL")
    
    def get_waypoint_altitude(self):
        """Calculate the altitude to use for new waypoints based on current mode"""
        if self.waypoint_altitude_mode == "CURRENT":
            return self.current_alt
        elif self.waypoint_altitude_mode == "RELATIVE":
            return self.current_alt + self.relative_altitude_offset
        elif self.waypoint_altitude_mode == "ABSOLUTE":
        ...
        while self.running:
            try:
                ...
                if msg_type == 'GPS_RAW_INT':
                    ...
                    self.current_lat = msg.lat / 1e7
                    self.current_lon = msg.lon / 1e7
                    # Use relative altitude (above home) for better accuracy
                    self.current_alt = msg.relative_alt / 1000.0  # Convert mm to meters
    ...
    def add_detection_point(self, x_meters, y_meters, z_meters, confidence):
        ...
        # UPDATED: Use dynamic altitude calculation instead of fixed offset
        waypoint_alt = self.get_waypoint_altitude()
        ...
        gps_point = {
            ...
            'vehicle_alt': self.current_alt,  # NEW: Store current vehicle altitude
            'vehicle_heading': self.current_heading,
            'altitude_mode': self.waypoint_altitude_mode  # NEW: Store which altitude mode was used
        }
        ...
        if success:
            ...
            # UPDATED: Show altitude information in the log message
            print(f"🎯 New waypoint sent: {distance:.1f}m away at bearing {bearing:.0f}°, altitude {waypoint_alt:.1f}m ({self.waypoint_altitude_mode})")
                    ...
                    if msg_type in ['MISSION_REQUEST', 'MISSION_REQUEST_INT']:
                        ...
                        if requested_seq not in items_sent:
                            if requested_seq == 0:
                                ...
                                # UPDATED: Use the calculated altitude for the waypoint
                        ...
                        if msg.type == mavutil.mavlink.MAV_MISSION_ACCEPTED:
                            print(f"✅ Waypoint uploaded successfully at {alt:.1f}m altitude!")
                            return True
                        else:
    ...
    def get_status(self):
        return {
            ...
            'altitude_mode': self.waypoint_altitude_mode,  # NEW: Include altitude mode in status
            'waypoint_altitude': self.get_waypoint_altitude()  # NEW: Show what altitude would be used
        }
...
# Remove the fixed altitude offset since we're now using dynamic altitude
# WAYPOINT_ALTITUDE_OFFSET = 0.0  # This line can be removed

# Optional: Add new configuration options for altitude modes
DEFAULT_ALTITUDE_MODE = "CURRENT"  # Options: "CURRENT", "RELATIVE", "ABSOLUTE"
DEFAULT_RELATIVE_OFFSET = 10.0     # meters above current altitude
DEFAULT_ABSOLUTE_ALTITUDE = 100.0  # fixed altitude MSL
...
def initialize_mavlink_with_altitude_config():
    """Initialize MAVLink with altitude configuration options"""
    mavlink_handler = None
    try:
        mavlink_handler = MAVLinkGPSHandler()
        
        # Set the default altitude mode
        mavlink_handler.set_waypoint_altitude_mode(DEFAULT_ALTITUDE_MODE, DEFAULT_RELATIVE_OFFSET)
        
        # You can change the altitude mode at runtime:
        # mavlink_handler.set_waypoint_altitude_mode("RELATIVE", 15.0)  # 15m above current
        # mavlink_handler.set_waypoint_altitude_mode("ABSOLUTE", 120.0) # 120m MSL
        # mavlink_handler.set_waypoint_altitude_mode("CURRENT")         # Same as aircraft
        
    except Exception as e:
        print(f"⚠️  MAVLink initialization failed: {e}")
        print("   Continuing without GPS waypoint functionality")
    
    return mavlink_handler


# ==============================================================================
# REPLACE YOUR MAIN INITIALIZATION WITH THIS:
# ==============================================================================

print("Initializing ULTRA-FAST servo system with GPS waypoint guidance...")

# Initialize MAVLink connection with altitude configuration
mavlink_handler = initialize_mavlink_with_altitude_config()
